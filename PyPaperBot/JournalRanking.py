...
class JournalRanker:
    ...
    def get_metrics(self, journal_title: str, score_cutoff=90) -> dict | None:
        ...
        Args:
            journal_title (str): The name of the journal to look up.
            score_cutoff (int): The minimum matching score (0-100) to consider a match valid.

        Returns:
            dict | None: A dictionary with 'SJR', 'H index', and 'SJR Best Quartile'
                        if a good match is found, otherwise None.
        """
        if self.df is None or not journal_title:
            return None

        # Pre-process the input string to handle HTML entities
        cleaned_title = html.unescape(journal_title)
